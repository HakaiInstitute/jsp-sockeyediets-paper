...
samplesizes <- data.frame(x=c("A1", "A2", "B1", "B2", "B3"), y=c(1.05, 1.05, 1.05, 1.05, 1.05), z=c(112, 6, 30, 23, 16))
...
dietcomp_cluster_plot <- ggplot(data = dietcomp_temp, aes(x = cluster, y = meanprop)) + 
        ...
        scale_y_continuous(limits = c(0, 1.07), breaks = seq(0, 1, by = 0.25), expand = c(0,0.02))
...
ggarrange(dietcomp_cluster_plot, sizes_plot, timing_comp_clusterplot, ncol = 1, common.legend = FALSE, legend = "right", labels = c("i", "ii", "iii"), label.x = 0.1, label.y = 1.05, nrow = 3, align = "v") +
  theme(plot.margin = margin(0.5,0.2,0.2,0.2, "cm"))
ggsave("../figs/speciescomp/temp_clustercomp.png", width = 20, height = 22, units = "cm", dpi = 300)
```
...
anova <- aov(gfi~location*as.factor(year)*tperiod, data = gfi3)
summary(anova)
plot(anova)
